...
int main(int argc,char *argv[]) {
	...
	cout << "Please enter the trace file name (*.pout or *.vout): " + filename << endl;
	...
	count = 0;
	float scaler{ 1000 / 90 }; //140, 106,165,142, 117, 108
								   //124, 165, 142, 117,108
	...
	for (auto& i : tm) {
		...
		outputfile << (uint64_t) ((i.first - global_start_time)* scaler) <<i.second << endl;
		...
		if (count != 1) sum_of_time += (uint64_t) ((i.first - global_start_time - prevtime)* scaler);
	...
	std::cout << "Application End Time(ns): " << (uint64_t) (prevtime*scaler) << endl;
	std::cout << "Memory Footprint: " << mfootprint.size() * 4096 / 1024 / 1024 << "MB" << endl;
